...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, num_skills, obs_space, cent_obs_space, act_space):
        ...
        self.z_onehot = np.zeros(
                    (self.episode_length + 1, self.n_rollout_threads, num_agents, self.num_skills), 
                    dtype=np.float32)
        self.rnn_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, self.recurrent_N, self.hidden_size),
            dtype=np.float32)
        self.rnn_states_critic = np.zeros_like(self.rnn_states)
        self.rnn_discriminator_states = np.zeros_like(self.rnn_states)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, rnn_discriminator_states, z_onehot, actions, action_log_probs,
        ...
        self.rnn_discriminator_states[self.step + 1] = rnn_discriminator_states.copy()
        self.z_onehot[self.step + 1] = z_onehot.copy()
    ...
    def chooseinsert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
        ...
        :param rnn_states_critic: (np.ndarray) RNN states for critic network.
        :param actions:(np.ndarray) actions taken by agents.
        :param action_logprobs:(np.ndarray) log probs of actions taken by agents
        :param value_preds: (np.ndarray) value function prediction at each step.
        :param rewards: (np.ndarray) reward collected at each step.
        :param masks: (np.ndarray) denotes whether the environment has terminated or not.
    ...
    def after_update(self):
        ...
        self.rnn_discriminator_states[0] = self.rnn_discriminator_states[-1].copy()
        self.masks[0] = self.masks[-1].copy()
        self.bad_masks[0] = self.bad_masks[-1].copy()
        self.active_masks[0] = self.active_masks[-1].copy()
        self.z_onehot[0] = self.z_onehot[-1].copy()
        if self.available_actions is not None:
            self.available_actions[0] = self.available_actions[-1].copy()

    def chooseafter_update(self):
        """Copy last timestep data to first index. This method is used for Hanabi."""
        self._rnn_states[0] = self.rnn_states[-1].copy()
        self.rnn_states_critic[0] = self.rnn_states_critic[-1].copy()
        self.rnn_states_critic[0] = self.rnn_states_critic[-1].copy()
        self.masks[0] = self.masks[-1].copy()
        self.bad_masks[0] = self.bad_masks[-1].copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        z_onehot = self.z_onehot[:-1].reshape(-1, *self.z_onehot.shape[3:])
        rnn_states = self.rnn_states[:-1].reshape(-1, *self.rnn_states.shape[3:])
        rnn_states_critic = self.rnn_states_critic[:-1].reshape(-1, *self.rnn_states_critic.shape[3:])
        rnn_discriminator_states = self.rnn_discriminator_states[:-1].reshape(-1, *self.rnn_discriminator_states.shape[3:])
        ...
        for indices in sampler:
            ...
            rnn_discriminator_states_batch = rnn_discriminator_states[indices]
            z_onehot_batch = z_onehot[indices]
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, rnn_discriminator_states_batch, \
                            actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, \
                            old_action_log_probs_batch, adv_targ, available_actions_batch, z_onehot_batch
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        rnn_discriminator_states = self.rnn_discriminator_states.reshape(-1, batch_size, *self.rnn_discriminator_states.shape[3:])
        z_onehot = self.z_onehot.reshape(-1, batch_size, *self.z_onehot.shape[3:])
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            rnn_discriminator_states_batch = []
            z_onehot_batch = []
            actions_batch = []
            ...
            for offset in range(num_envs_per_batch):
                ...
                rnn_discriminator_states_batch.append(rnn_discriminator_states[0:1, ind])
                z_onehot_batch.append(z_onehot[:-1, ind])                
            ...
            rnn_discriminator_states_batch = np.stack(rnn_discriminator_states_batch).reshape(N, *self.rnn_discriminator_states.shape[3:])

            # Flatten the (T, N, ...) from_numpys to (T * N, ...)
            share_obs_batch = _flatten(T, N, share_obs_batch)
            obs_batch = _flatten(T, N, obs_batch)
            z_onehot_batch = _flatten(T, N, z_onehot_batch)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        # Thêm z_onehot
        z_onehot = _cast(self.z_onehot[:-1])
        ...
        rnn_discriminator_states = self.rnn_discriminator_states[:-1].transpose(1, 2, 0, 3, 4).reshape(-1, *self.rnn_discriminator_states.shape[3:])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                z_onehot_batch.append(z_onehot[ind:ind + data_chunk_length])
                ...
                rnn_discriminator_states_batch.append(rnn_discriminator_states[ind])

            L, N = data_chunk_length, mini_batch_size

            share_obs_batch = np.stack(share_obs_batch, axis=1)
            obs_batch = np.stack(obs_batch, axis=1)
            z_onehot_batch = np.stack(z_onehot_batch, axis=1)
            ...
            z_onehot_batch = _flatten(L, N, z_onehot_batch)
